...
class MaterialService
{
    ...
    private function createReceiveTransaction(Material $material, array $data): void
    {
        WarehouseTransaction::create([
            ...
            'notes' => 'استقبال مادة جديدة - ' . $material->material_type,
            'notes_en' => 'Receiving new material - ' . $material->material_type_en,
            'created_by' => auth()->id(),
            'approved_at' => now(),
        ]);
    }
    ...
    private function handleWeightChange(Material $material, float $oldWeight, float $newWeight): void
    {
        ...
        if ($weightDifference > 0) {
            ...
            WarehouseTransaction::create([
                ...
                'unit_id' => $material->unit_id,
                'reference_number' => $material->delivery_note_number ?? null,
                'notes' => 'إضافة كمية للمادة - ' . $material->material_type,
                'notes_en' => 'Adding quantity to material - ' . $material->material_type_en,
                'created_by' => auth()->id(),
                'approved_at' => now(),
            ]);
            ...
            WarehouseTransaction::create([
                ...
                'unit_id' => $material->unit_id,
                'notes' => 'تعديل كمية المادة (طرح) - ' . $material->material_type,
                'notes_en' => 'Material quantity adjustment (deduction) - ' . $material->material_type_en,
                'created_by' => auth()->id(),
                'approved_at' => now(),
            ]);
        }
    ...
    private function handleRemainingWeightChange(Material $material, float $oldRemaining, float $newRemaining): void
    {
        ...
        if ($remainingDifference > 0) {
            ...
            WarehouseTransaction::create([
                ...
                'unit_id' => $material->unit_id,
                'notes' => 'صرف مادة - ' . $material->material_type,
                'notes_en' => 'Material issue - ' . $material->material_type_en,
                'created_by' => auth()->id(),
                'approved_at' => now(),
            ]);
            ...
            WarehouseTransaction::create([
                ...
                'unit_id' => $material->unit_id,
                'notes' => 'تعديل الكمية المتبقية - ' . $material->material_type,
                'notes_en' => 'Adjustment of remaining quantity - ' . $material->material_type_en,
                'created_by' => auth()->id(),
                'approved_at' => now(),
            ]);
        }
    ...
    private function handleWarehouseChange(Material $material, int $oldWarehouseId, int $newWarehouseId): void
    {
        if ($material->remaining_weight > 0) {
            ...
            WarehouseTransaction::create([
                ...
                'notes' => 'نقل مادة من المستودع - ' . $material->material_type,
                'notes_en' => 'Transfer material from warehouse - ' . $material->material_type_en,
                'created_by' => auth()->id(),
                'approved_at' => now(),
            ]);
        }
