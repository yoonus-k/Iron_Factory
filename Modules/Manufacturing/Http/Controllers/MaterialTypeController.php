...
class MaterialTypeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // جلب الترجمات لكل مادة
        foreach ($materialTypes as $materialType) {
            $materialType->translations = $materialType->getAllTranslations();
        }

        return view('manufacturing::warehouses.settings.material-types.index', compact('materialTypes', 'locale'));
    }
    ...
    public function create()
    {
        ...
        $autoGeneratedCode = $this->generateTypeCode();
        $locale = App::getLocale(); // الحصول على اللغة الحالية

        return view('manufacturing::warehouses.settings.material-types.create', compact('categories', 'units', 'autoGeneratedCode', 'locale'));
    }

    private function generateTypeCode()
    {
        $prefix = 'MT-';
        $lastType = MaterialType::where('type_code', 'like', $prefix . '%')
            ->orderBy('type_code', 'desc')
            ->first();

        if ($lastType) {
            $lastNumber = (int) substr($lastType->type_code, strlen($prefix));
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (empty($typeCode)) {
                $typeCode = $this->generateTypeCode();
            }
            ...
            $translations = [
                'en' => [
                    'name' => $validated['type_name_en'] ?? $validated['type_name'],
                    'description' => $validated['description_en'] ?? $validated['description'] ?? '',
                    'storage_conditions' => $validated['storage_conditions_en'] ?? $validated['storage_conditions'] ?? '',
                ],
                'ar' => [
            ...
            TranslationHelper::saveForAllLanguages('App\Models\MaterialType', $materialType->id, $translations);
    ...
    public function show($id)
    {
        ...
        // جلب الترجمات
        $translations = $materialType->getAllTranslations();

        return view('manufacturing::warehouses.settings.material-types.show', compact('materialType', 'locale', 'translations'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('manufacturing::warehouses.settings.material-types.edit', compact('materialType', 'categories', 'units', 'locale', 'translations'));
    }
