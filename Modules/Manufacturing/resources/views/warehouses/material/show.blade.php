    ...
    <div class="container">
                ...
                <div class="header-actions">
                    ...
                    @if (auth()->user()->hasPermission('WAREHOUSE_MATERIALS_UPDATE'))
                        <div class="um-dropdown">
                            <button class="btn " type="button" data-bs-toggle="dropdown" title="{{ __('warehouse.change_status') }}">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" >
        ...
        <div class="grid">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="info-item">
                        <div class="info-label">{{ __('warehouse.material_code') }}:</div>
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            @if ($material->status === 'available')
                                <span class="badge badge-success">{{ __('warehouse.available') }}</span>
                            @elseif ($material->status === 'in_use')
                                <span class="badge badge-warning">{{ __('warehouse.in_use') }}</span>
                            @elseif ($material->status === 'consumed')
                                <span class="badge badge-danger">{{ __('warehouse.consumed') }}</span>
                            @else
                                <span class="badge badge-secondary">{{ __('warehouse.expired') }}</span>
                            @endif
                        </div>
                    </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="info-item">
                        <div class="info-label">Shelf Location (EN):</div>
                ...
                <div class="card-body">
                            ...
                            @foreach($warehouseDetails as $warehouseId => $details)
                                ...
                                <div style="border: 1px solid #e9ecef; border-radius: 8px; padding: 15px; background: #f8f9fa;">
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        <div>
                                            @if($isEmpty)
                                                <span class="badge badge-danger">{{ __('warehouse.empty') }}</span>
                                            @elseif($percentageRemaining < 30)
                                                <span class="badge badge-warning">{{ __('warehouse.low_quantity') }}</span>
                                            @else
                                                <span class="badge badge-success">{{ __('warehouse.available') }}</span>
                                            @endif
                                        </div>
                                        <div>
                                            <small style="color: #7f8c8d; font-size: 11px;">{{ __('warehouse.showing') }} {{ $details->count() }}</small>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    @else
                        <div style="text-align: center; padding: 60px 20px; color: #95a5a6;">
                            ...
                            <p style="margin: 0; font-size: 16px; font-weight: 500;">{{ __('warehouse.no_quantities_found') }}</p>
                        </div>
                    @endif
                ...
                <div class="card-body">
                    ...
                    @else
                        <div style="text-align: center; padding: 40px 20px; color: #95a5a6;">
                            ...
                            <p style="margin: 0; font-size: 14px;">{{ __('warehouse.no_recent_activity') }}</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card" style="margin-bottom: 20px;">
                <div class="card-header">
                    ...
                    <h3 class="card-title">{{ __('warehouse.movement_history') }}</h3>
                </div>
                <div class="card-body">
                    <!-- فلاتر البحث -->
                    <form method="GET" action="{{ route('manufacturing.warehouses.material.show', $material->id) }}" class="mb-4">
                        <div class="row g-3">
                            <div class="col-md-3">
                                <label class="form-label" style="font-weight: 600; color: var(--secondary-gray); font-size: 14px;">{{ __('warehouse.movement_type') }}</label>
                                <select name="movement_type" class="form-select" style="border: 2px solid #e9ecef; border-radius: 8px;">
                                    ...
                                    <option value="in" {{ request('movement_type') == 'in' ? 'selected' : '' }}>{{ __('warehouse.movement_in') }}</option>
                                    <option value="out" {{ request('movement_type') == 'out' ? 'selected' : '' }}>{{ __('warehouse.movement_out') }}</option>
                                    <option value="transfer" {{ request('movement_type') == 'transfer' ? 'selected' : '' }}>{{ __('warehouse.movement_transfer') }}</option>
                                    <option value="adjustment" {{ request('movement_type') == 'adjustment' ? 'selected' : '' }}>{{ __('warehouse.movement_adjustment') }}</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label" style="font-weight: 600; color: var(--secondary-gray); font-size: 14px;">{{ __('warehouse.warehouse_name') }}</label>
                            ...
                            <div class="col-md-2">
                                <label class="form-label" style="font-weight: 600; color: var(--secondary-gray); font-size: 14px;">{{ __('warehouse.created_at') }}</label>
                            ...
                            <div class="col-md-2">
                                <label class="form-label" style="font-weight: 600; color: var(--secondary-gray); font-size: 14px;">{{ __('warehouse.updated_at') }}</label>
                            ...
                            <div class="col-md-2 d-flex align-items-end gap-2">
                                <button type="submit" class="btn btn-primary " >
                                    <i class="fas fa-search"></i> {{ __('warehouse.search') }}
                                </button>
                        ...
                        <div class="table-responsive">
                            <table class="table" style="margin: 0;">
                                <thead style="background: #f8f9fa;">
                                    <tr>
                                        <th style="padding: 12px; font-size: 12px; color: #7f8c8d; font-weight: 600;">{{ __('warehouse.movement_date') }}</th>
                                        ...
                                        <th style="padding: 12px; font-size: 12px; color: #7f8c8d; font-weight: 600;">{{ __('warehouse.destination') }}</th>
                                        <th style="padding: 12px; font-size: 12px; color: #7f8c8d; font-weight: 600;">{{ __('warehouse.quantity') }}</th>
                                        <th style="padding: 12px; font-size: 12px; color: #7f8c8d; font-weight: 600;">{{ __('warehouse.supplier') }}</th>
                                ...
                                <tbody>
                                    ...
                                    <tr style="border-bottom: 1px solid #e9ecef;">
                                        ...
                                        <td style="padding: 12px;">
                                            @php
                                                $typeColors = [
                                                    'in' => ['bg' => '#d4edda', 'text' => '#155724', 'label' => __('warehouse.movement_in')],
                                                    'out' => ['bg' => '#f8d7da', 'text' => '#721c24', 'label' => __('warehouse.movement_out')],
                                                    'transfer' => ['bg' => '#d1ecf1', 'text' => '#0c5460', 'label' => __('warehouse.movement_transfer')],
                                                    'adjustment' => ['bg' => '#fff3cd', 'text' => '#856404', 'label' => __('warehouse.movement_adjustment')]
                                                ];
                        ...
                        <div style="padding: 20px; border-top: 1px solid #e9ecef; background: #f8f9fa;">
                            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                                <div style="font-size: 14px; color: #7f8c8d;">
                                    {{ __('warehouse.showing') }} <strong>{{ $movements->firstItem() }}</strong> {{ __('warehouse.to') }} <strong>{{ $movements->lastItem() }}</strong> {{ __('warehouse.of') }} <strong>{{ $movements->total() }}</strong>
                                </div>
                                <div>
                    ...
                    @else
                        <div style="text-align: center; padding: 60px 20px; color: #95a5a6;">
                            ...
                            <p style="margin: 0; font-size: 16px; font-weight: 500;">{{ __('warehouse.no_movements_found') }}</p>
                            <small style="color: #bdc3c7; display: block; margin-top: 8px;">{{ __('warehouse.no_recent_activity') }}</small>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="warehouse-info-container" id="warehouseInfoContainer" style="display: none;">
                            <div class="warehouse-card">
                                <div class="warehouse-card-header">
                                    ...
                                    <h5>{{ __('warehouse.warehouse_info') }}</h5>
                                </div>
                                <div class="warehouse-card-body">
                                    <div class="warehouse-detail">
                                        <span class="warehouse-label">{{ __('warehouse.warehouse_name') }}:</span>
                                        <span class="warehouse-value" id="warehouseName">-</span>
                                    </div>
                                    <div class="warehouse-detail">
                                        <span class="warehouse-label">{{ __('warehouse.current_quantity') }}:</span>
                                    ...
                                    <div class="warehouse-detail status">
                                        <span class="warehouse-label">{{ __('warehouse.status') }}:</span>
                        ...
                        <div class="form-group">
                            <label for="notes" class="form-label">
                                ...
                                {{ __('warehouse.notes_optional') }}
                            </label>
                            <textarea name="notes" id="notes" class="form-control" rows="3"
                                    placeholder="{{ __('warehouse.enter_notes') }}..."></textarea>
                        </div>
                        ...
                        <div class="alert alert-info" role="alert">
                            <h6 class="alert-heading">
                                ...
                                {{ __('warehouse.important_information') }}
                            </h6>
                            <ul style="margin: 10px 0 0 0; padding-left: 20px; font-size: 13px; line-height: 1.8;">
                                <li>{{ __('warehouse.warehouse_will_be_updated') }}</li>
                                <li>{{ __('warehouse.movement_will_be_recorded') }}</li>
                                <li>{{ __('warehouse.user_will_be_saved') }}</li>
                                <li>{{ __('warehouse.datetime_will_be_saved') }}</li>
                            </ul>
                        </div>
                    </div>
    ...
    <script>
        ...
        function updateWarehouseInfo() {
            ...
            if (quantity > 10) {
                ...
                statusElement.innerHTML = '<span class="status-dot"></span><span class="status-text">{{ __('warehouse.available') }}</span>';
            } else if (quantity > 0) {
                statusElement.className = 'warehouse-status-badge low';
                statusElement.innerHTML = '<span class="status-dot"></span><span class="status-text">{{ __('warehouse.low_quantity') }}</span>';
            } else {
                statusElement.className = 'warehouse-status-badge empty';
                statusElement.innerHTML = '<span class="status-dot"></span><span class="status-text">{{ __('warehouse.empty') }}</span>';
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (addQuantityForm) {
                addQuantityForm.addEventListener('submit', function(e) {
                    ...
                    if (!warehouseId) {
                        e.preventDefault();
                        alert('⚠️ {{ __('warehouse.please_select') }} {{ __('warehouse.select_warehouse') }}');
                    ...
                    if (!quantity || parseFloat(quantity) <= 0) {
                        e.preventDefault();
                        alert('⚠️ {{ __('warehouse.please_select') }} {{ __('warehouse.quantity_to_add') }}');
                    ...
                    if (!confirm(`✅ {{ __('warehouse.confirm_add_quantity') }}: ${quantity} {{ __('warehouse.unit') }} {{ __('warehouse.to') }} "${warehouseName}"?`)) {
                        e.preventDefault();
                        return false;
                    }
                });
                ...
                button.addEventListener('click', function(e) {
                    ...
                    if (typeof Swal !== 'undefined') {
                        Swal.fire({
                            ...
                            text: '{{ __('warehouse.delete_material_warning') }}',
                            icon: 'warning',
                        ...
                        if (confirm('{{ __('warehouse.delete_material_warning') }}')) {
                            form.submit();
                        }
                    }
                });
