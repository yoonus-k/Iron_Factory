...
@section('content')
    ...
    <!-- مؤشر التقدم -->
    <div class="um-alert-custom um-alert-info" style="display: flex; align-items: center; gap: 20px; margin-bottom: 24px; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: none; color: white;">
        <div style="display: flex; align-items: center; gap: 15px; flex: 1;">
            <div style="background: rgba(255,255,255,0.3); color: white; width: 45px; height: 45px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 20px; border: 3px solid white;">
                1
            </div>
            <div style="flex: 1;">
                <strong style="font-size: 16px; display: block; margin-bottom: 5px;">📋 الخطوة الأولى: إنشاء أذن التسليم</strong>
                <small style="opacity: 0.9;">بعد الحفظ، ستنتقل تلقائياً لخطوة التسجيل في المستودع وتوليد الباركود</small>
            </div>
        </div>
        <div style="display: flex; align-items: center; gap: 10px; opacity: 0.6;">
            <div style="width: 35px; height: 35px; border-radius: 50%; border: 2px dashed rgba(255,255,255,0.5); display: flex; align-items: center; justify-content: center; font-size: 16px;">
                2
            </div>
            <i class="feather icon-arrow-left" style="font-size: 20px;"></i>
            <span style="font-size: 13px;">التسجيل والباركود</span>
        </div>
    </div>
        ...
        <div class="form-section warehouse-only">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <small style="color: #7f8c8d; margin-top: 5px;">يتم توليده تلقائياً - لا تقلق من الرقم الظاهر هنا، سيتم توليد رقم جديد تلقائياً عند الحفظ</small>
                </div>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <div style="margin-top: 10px; padding: 12px; background: #f8f9fa; border-radius: 6px; border-right: 3px solid #27ae60;">
                        <small style="color: #27ae60;" id="incoming_material_display">اختر المستودع أولاً</small>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Populate incoming materials based on warehouse
            incomingWarehouse.addEventListener('change', function() {
                const warehouseId = this.value;
                incomingMaterial.innerHTML = '<option value="">اختر المادة</option>';
                ...
                if (warehouseId && Array.isArray(materialDetails)) {
                    ...
                    if (filtered.length > 0) {
                        filtered.forEach(material => {
                            ...
                            option.text = `${material.name_ar || material.name_ar} (${material.quantity} ${material.unit_name || 'كيلو'})`;
                            incomingMaterial.appendChild(option);
                        });
                        incomingMaterialDisplay.innerHTML = '✓ تم تحميل المواد المتاحة';
                    } else {
            ...
            // Update material display when incoming material changes
            incomingMaterial.addEventListener('change', function() {
                const selectedOption = this.options[this.selectedIndex];
                if (this.value) {
                    const quantity = selectedOption.getAttribute('data-quantity');
                    const unit = selectedOption.getAttribute('data-unit');
                    incomingMaterialDisplay.innerHTML = `✓ متوفر في المستودع: <strong>${quantity} ${unit}</strong>`;
                } else {
                    incomingMaterialDisplay.innerHTML = 'اختر المادة لعرض التفاصيل';
                }
            });
