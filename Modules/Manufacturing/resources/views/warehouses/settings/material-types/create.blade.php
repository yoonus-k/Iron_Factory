...
@section('title', __('warehouse.add_new_material_type'))
...
@section('content')
    ...
    <div class="um-header-section">
        ...
        <nav class="um-breadcrumb-nav">
            ...
            <i class="feather icon-chevron-left"></i>
            <span>{{ __('warehouse.add_new_material_type') }}</span>
        </nav>
    </div>
        ...
        <form method="POST" action="{{ route('manufacturing.warehouse-settings.material-types.store') }}" id="materialTypeForm">
            ...
            <div class="form-section">
                <div class="section-header">
                    ...
                    <div>
                        ...
                        <p class="section-subtitle">{{ __('warehouse.enter_material_type_data') }}</p>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <input type="text" name="type_code" id="type_code" class="form-input @error('type_code') error @enderror"
                                   placeholder="{{ __('warehouse.will_be_generated_automatically') }}" value="{{ old('type_code', $autoGeneratedCode ?? '') }}" readonly required>
                        </div>
                        @error('type_code')
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            <select name="category" id="category" class="form-input @error('category') error @enderror" required>
                                ...
                                <option value="raw_material" {{ old('category') == 'raw_material' ? 'selected' : '' }}>{{ __('warehouse.raw_material_category') }}</option>
                                <option value="finished_product" {{ old('category') == 'finished_product' ? 'selected' : '' }}>{{ __('warehouse.finished_product_category') }}</option>
                                <option value="semi_finished" {{ old('category') == 'semi_finished' ? 'selected' : '' }}>{{ __('warehouse.semi_finished_category') }}</option>
                                <option value="additive" {{ old('category') == 'additive' ? 'selected' : '' }}>{{ __('warehouse.additive_category') }}</option>
                                <option value="packing_material" {{ old('category') == 'packing_material' ? 'selected' : '' }}>{{ __('warehouse.packing_material_category') }}</option>
                                <option value="component" {{ old('category') == 'component' ? 'selected' : '' }}>{{ __('warehouse.component_category') }}</option>
                            </select>
                        </div>
                        @error('category')
                            <div class="error-message" style="display: block;">{{ $message }}</div>
                    ...
                    <div class="form-group full-width" id="description-group-ar" style="@if($locale !== 'ar') display: none; @endif">
                        ...
                        <div class="input-wrapper">
                            <textarea name="description" id="description" class="form-input @error('description') error @enderror"
                                      placeholder="{{ __('warehouse.enter_description') }}" rows="3" maxlength="1000">{{ old('description') }}</textarea>
                        </div>
                        @error('description')
                            <div class="error-message" style="display: block;">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group full-width" id="description-group-en" style="@if($locale === 'ar') display: none; @endif">
                        ...
                        <div class="input-wrapper">
                            <textarea name="description_en" id="description_en" class="form-input @error('description_en') error @enderror"
                                      placeholder="{{ __('warehouse.enter_description_en') }}" rows="3" maxlength="1000">{{ old('description_en') }}</textarea>
                        </div>
                    ...
                    <div class="form-group full-width" id="storage-group-ar" style="@if($locale !== 'ar') display: none; @endif">
                        ...
                        <div class="input-wrapper">
                            <textarea name="storage_conditions" id="storage_conditions" class="form-input @error('storage_conditions') error @enderror"
                                      placeholder="{{ __('warehouse.storage_conditions_example') }}" rows="2">{{ old('storage_conditions') }}</textarea>
                        </div>
                    ...
                    <div class="form-group full-width" id="storage-group-en" style="@if($locale === 'ar') display: none; @endif">
                        ...
                        <div class="input-wrapper">
                            <textarea name="storage_conditions_en" id="storage_conditions_en" class="form-input @error('storage_conditions_en') error @enderror"
                                      placeholder="{{ __('warehouse.storage_conditions_example_en') }}" rows="2">{{ old('storage_conditions_en') }}</textarea>
                        </div>
            ...
            <div class="form-actions">
                <button type="submit" class="btn-submit">
                    ...
                    {{ __('warehouse.save_material_type') }}
                </button>
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (isValid) {
                    ...
                    Swal.fire({
                        ...
                        text: '{{ __('warehouse.confirm_save_material_type') }}',
                        icon: 'question',
