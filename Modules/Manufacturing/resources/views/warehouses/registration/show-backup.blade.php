...
@if (session('success'))
    <div class="um-alert-custom um-alert-success" role="alert" id="successMessage" style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); border: none; color: white; padding: 25px; border-radius: 15px; box-shadow: 0 10px 30px rgba(17, 153, 142, 0.3);">
        <div style="display: flex; align-items: center; gap: 20px;">
            <div style="font-size: 48px;">✅</div>
            <div style="flex: 1;">
                <div style="font-size: 18px; font-weight: bold; margin-bottom: 8px;">{{ session('success') }}</div>
                @if(session('batch_code'))
                    <div style="background: rgba(255,255,255,0.2); padding: 15px; border-radius: 10px; margin-top: 12px; border: 2px dashed rgba(255,255,255,0.5);">
                        <div style="font-size: 13px; opacity: 0.9; margin-bottom: 5px;">🏷️ رقم الدفعة (الباركود):</div>
                        ...
                        <div style="margin-top: 10px; font-size: 13px; opacity: 0.9;">
                            يمكنك الآن استخدام هذا الباركود لتتبع المنتج من المرحلة الأولى
                        </div>
                    </div>
                @endif
            </div>
            <button type="button" class="um-alert-close" onclick="this.parentElement.parentElement.style.display='none'" style="color: white; opacity: 0.8;">
    ...
    <div class="grid">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="info-item" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 25px; border-radius: 12px; margin: 15px 0; box-shadow: 0 5px 20px rgba(102, 126, 234, 0.4);">
                    <div style="display: flex; flex-direction: column; gap: 20px; color: white;">
                        <div style="text-align: center;">
                            <div style="font-size: 16px; opacity: 0.9; margin-bottom: 12px;">🏷️ باركود الدفعة - المستودع</div>
                            <svg id="warehouse-barcode" style="background: white; padding: 15px; border-radius: 10px; margin: 0 auto;"></svg>
                            <div style="font-size: 24px; font-weight: bold; letter-spacing: 3px; font-family: 'Courier New', monospace; margin-top: 15px;">
                                {{ $deliveryNote->materialBatch->batch_code }}
                            </div>
                        </div>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; background: rgba(255,255,255,0.15); padding: 15px; border-radius: 8px;">
                            <div>
                                <div style="font-size: 12px; opacity: 0.8;">المادة:</div>
                                <div style="font-weight: bold;">{{ $deliveryNote->material->name_ar ?? 'غير محدد' }}</div>
                            </div>
                            <div>
                                <div style="font-size: 12px; opacity: 0.8;">الكمية:</div>
                                <div style="font-weight: bold;">{{ number_format($deliveryNote->materialBatch->initial_quantity, 2) }} {{ $deliveryNote->materialBatch->unit->unit_name ?? 'كجم' }}</div>
                            </div>
                            <div>
                                <div style="font-size: 12px; opacity: 0.8;">المورد:</div>
                                <div style="font-weight: bold;">{{ $deliveryNote->supplier->name ?? 'غير محدد' }}</div>
                            </div>
                            <div>
                                <div style="font-size: 12px; opacity: 0.8;">تاريخ التوليد:</div>
                                <div style="font-weight: bold;">{{ $deliveryNote->materialBatch->created_at->format('Y-m-d H:i') }}</div>
                            </div>
                        </div>
                        <button onclick="printWarehouseBarcode('{{ $deliveryNote->materialBatch->batch_code }}', '{{ $deliveryNote->note_number }}', '{{ $deliveryNote->material->name_ar ?? 'غير محدد' }}', {{ $deliveryNote->materialBatch->initial_quantity }}, '{{ $deliveryNote->supplier->name ?? 'غير محدد' }}')" 
                                style="background: white; border: none; color: #667eea; padding: 14px 30px; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.3s; font-size: 16px; display: flex; align-items: center; justify-content: center; gap: 10px;">
