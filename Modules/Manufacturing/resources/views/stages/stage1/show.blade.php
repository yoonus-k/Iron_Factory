            ...
            <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                ...
                <a href="{{ route('manufacturing.shift-handovers.transfer-stage', ['stage_record_id' => $stand->id, 'stage_number' => 1, 'from_shift_id' => $currentShift->id]) }}"
                   class="um-btn um-btn-success" style="white-space: nowrap;">
                    <i class="feather icon-send"></i> نقل المرحلة
                </a>

                <!-- زر نقل العمال -->
                <a href="{{ route('manufacturing.shift-handovers.transfer-workers', ['from_shift_id' => $currentShift->id]) }}"
                   class="um-btn um-btn-primary" style="white-space: nowrap; background-color: #3498db;">
                    <i class="feather icon-users"></i> نقل العمال
                </a>
            </div>
            <small style="display: block; margin-top: 12px; color: #1565c0;">
                <i class="feather icon-info"></i>
                • اختر "نقل المرحلة" لنقل الستاند للوردية الأخرى
                <br>
                <i class="feather icon-info"></i>
                • اختر "نقل العمال" لتحويل العمال لورديات أخرى
            </small>
        </div>
        @endif
    </div>
        ...
        <div style="max-height: 400px; overflow-y: auto;">
            ...
            <div style="background: #fff; padding: 15px; margin-bottom: 12px; border-radius: 8px; border-left: 3px solid #f39c12;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
                    <div>
                        <strong style="color: #2c3e50; display: block; font-size: 16px;">{{ $shift->from_user_name }}</strong>
                        <small style="color: #7f8c8d;">من</small>
                    </div>
                    <i class="feather icon-arrow-right" style="color: #f39c12; margin: 5px 0;"></i>
                    <div style="text-align: right;">
                        <strong style="color: #2c3e50; display: block; font-size: 16px;">{{ $shift->to_user_name }}</strong>
                        <small style="color: #7f8c8d;">إلى</small>
                    </div>
                </div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; padding-top: 10px; border-top: 1px solid #eee;">
                    <div>
                        <small style="color: #7f8c8d;">وقت النقل:</small>
                        <span style="display: block; color: #2c3e50; font-weight: 600;">
                            {{ \Carbon\Carbon::parse($shift->handover_time)->format('Y-m-d H:i') }}
                        </span>
                    </div>
                    <div>
                        <small style="color: #7f8c8d;">الحالة:</small>
                        <span style="display: block;
                            @if($shift->supervisor_approved) background: #e8f5e9; color: #27ae60; @else background: #fff3cd; color: #f39c12; @endif
                            padding: 4px 8px; border-radius: 4px; font-weight: 600; font-size: 12px; text-align: center;">
                            @if($shift->supervisor_approved)
                                <i class="feather icon-check-circle"></i> موافق عليه
                            @else
                                <i class="feather icon-clock"></i> قيد الانتظار
                            @endif
                        </span>
                    </div>
                </div>
                ...
                <div style="margin-top: 10px; padding: 8px; background: #f8f9fa; border-radius: 4px;">
                    <small style="color: #7f8c8d;">ملاحظات:</small>
