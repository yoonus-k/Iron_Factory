            ...
            <div class="notifications-container">
                <div class="section-card">
                    <div class="section-header">
                        ...
                        <div class="header-badge">
                            <span class="count-badge badge-danger"
                                id="notifications-badge">{{ $notifications->count() }}</span>
                        </div>
                    </div>
                        ...
                        @forelse($notifications as $notification)
                            <div class="notification-card notification-{{ $notification['type'] }}">
                                <div class="notification-icon">
                                    <i
                                        class="fas fa-{{ $notification['type'] === 'warning' ? 'exclamation-triangle' : ($notification['type'] === 'success' ? 'check-circle' : 'times-circle') }}"></i>
                                </div>
    ...
    <script>
        $(document).ready(function () {
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            if (response.pending_confirmations) {
                                const highlightIds = (response.new_confirmations || []).map(c => c.id);
                                renderAllConfirmations(response.pending_confirmations, highlightIds);
                            } else if (response.new_confirmations.length > 0) {
            ...
            function mapStageCodeToName(stageCode) {
                return stageCodeNameMap[stageCode] || 'غير محدد';
            }

            function mapStageTypeToName(stageType) {
                return stageTypeNameMap[stageType] || null;
            }

            function getConfirmationTypeLabel(type, confirmation = {}) {
                if (confirmation.display_confirmation_type_label) {
                    return confirmation.display_confirmation_type_label;
                }
            ...
            function resolveMaterialName(confirmation) {
                if (confirmation.display_material_name) {
                    return confirmation.display_material_name;
                }
            ...
            function resolveBarcode(confirmation) {
                if (confirmation.display_production_barcode) {
                    return confirmation.display_production_barcode;
                }

                if (confirmation.resolved_barcode) {
                    return confirmation.resolved_barcode;
                }
                if (confirmation.metadata?.stage_barcode) {
                    return confirmation.metadata.stage_barcode;
                }
                if (confirmation.metadata?.barcode) {
                    return confirmation.metadata.barcode;
                }
            ...
            function resolveStageLabel(confirmation) {
                if (confirmation.display_stage_label) {
                    return confirmation.display_stage_label;
                }
            ...
            function resolveWeightLabel(confirmation) {
                if (confirmation.display_final_weight_label) {
                    return confirmation.display_final_weight_label;
                }

                if (confirmation.display_final_weight && parseFloat(confirmation.display_final_weight) > 0) {
                    return `${parseFloat(confirmation.display_final_weight).toFixed(2)} كجم`;
                }

                const candidates = [
                    confirmation.resolved_weight,
                    confirmation.metadata?.stage_weight,
            ...
            function createConfirmationHtml(confirmation, options = {}) {
                ...
                const typeLabel = getConfirmationTypeLabel(confirmation.confirmation_type, confirmation);
                const reason = confirmation.display_reason_text ?? confirmation.metadata?.reason;
                const notes = confirmation.notes;
                const highlightClass = options.highlight ? ' new-item' : '';
            ...
            ` : '';
                ...
                return `
                <div class="confirmation-card${highlightClass}" id="confirmation-${confirmation.id}">
            ...
            function renderAllConfirmations(confirmations, highlightIds = []) {
                ...
                confirmations.forEach(function (confirmation) {
                    const cardHtml = createConfirmationHtml(confirmation, {
                        highlight: highlightSet.has(confirmation.id)
                    });
                    $list.append(cardHtml);
                });

                const totalCards = confirmations.length;
                $('#pending-badge').text(totalCards);
                $('#stat-pending').text(totalCards);
            }

            // تحديث التأكيدات (للسيناريوهات التي لا تعيد القائمة كاملة)
            function updateConfirmations(confirmations) {
                ...
                confirmations.forEach(function (confirmation) {
                    ...
                    const cardHtml = createConfirmationHtml(confirmation, { highlight: true });
            ...
            $('#reject-submit').on('click', function (e) {
                ...
                $.ajax({
                    ...
                    contentType: 'application/json',
                    success: function (response) {
                        console.log('نجح الرفض:', response);
                    ...
                    error: function (xhr, status, error) {
                        console.error('خطأ AJAX في الرفض:');
